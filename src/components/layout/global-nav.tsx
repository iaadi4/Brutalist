  ...
  return (
    <header className="sticky top-0 z-50 w-full brutal-border-b bg-[var(--color-brutal-bg)] border-b-[6px] border-[var(--color-brutal-black)] shadow-[0px_4px_0px_#000]">
      <div className="flex flex-col md:flex-row w-full justify-between items-stretch">
        ...
        <nav className="w-full md:flex-1 flex overflow-x-auto overflow-y-hidden items-stretch scrollbar-thin scrollbar-thumb-[var(--color-brutal-black)] scrollbar-track-transparent active:scrollbar-thumb-[var(--color-brutal-red)]" style={{ touchAction: 'pan-x', WebkitOverflowScrolling: 'touch' }}>
          <ul className="flex flex-nowrap w-max md:w-full items-stretch h-full">
            {navLinks.map((link) => {
